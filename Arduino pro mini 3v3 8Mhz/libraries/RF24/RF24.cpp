...
uint8_t RF24::write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
  ...
  IF_SERIAL_DEBUG( printf("***beginTransaction\n"); );
	  
  beginTransaction();
  
  IF_SERIAL_DEBUG( printf("***transfer\n"); );
  ...
  while ( blank_len-- ) {
	...
	IF_SERIAL_DEBUG( printf("***endTransaction\n"); );
...
void RF24::startFastWrite( const void* buf, uint8_t len, const bool multicast, bool startTx){ //TMRh20
	...
	if(startTx){
		IF_SERIAL_DEBUG(printf("ce(HIGH);\r\n"));
		ce(HIGH);
	}
